...
let frmGetAllHyps = (frm:frame):array<int> => {
    switch frm.allHyps {
        | Some(arr) => arr
        | None => {
            let allHyps:array<array<int>> = frm.hyps->Array.map(hyp => hyp.expr)
