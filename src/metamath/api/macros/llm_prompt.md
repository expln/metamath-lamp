## The task
Your task is to help me develop a Metamath proof using the metamath-lamp (mm-lamp) tool.
Mm-lamp has built-in API functions which you can call to add/update/delete steps in the editor
and invoke different features of mm-lamp like the bottom-up prover, etc.
...
I will execute the function you asked for and reply to you with the output of the function.
The output of the function and all the previous conversation should help you come up with the next action
to take to get the proof complete.
...
* `v` - step is proved
* `?` - step is not proved
* `~` - step has a valid justification but some of its dependency steps are not proved yet
...
Not all steps will be returned. Only the steps listed below will be returned:
* bookmarked steps
...
You can always use existing (predefined) variables when you need them in the proof.
It makes sense to add only variables that are used in statements.
...
The meaning of all attributes of a step object is the same as for the `addSteps` function.
...
So, only attributes that need to be changed should be provided.
...
(saved in the editor).
If no proof is found then this function doesn't return anything.
In such cases I will write to you that no proof was found, and you will need to think about what to do next.
...
Skipping explicit justifications will reduce the number of errors caused by an incorrect assertion label
(caused by hallucination or if the assertion was renamed recently).
So, try not to provide explicit justifications and let mm-lamp figure them out.

### Use bottom-up prover

In addition to the skipping explicit justifications technique, you can skip some intermediate steps
and use the bottom-up prover to let mm-lamp find some simple/obvious intermediate steps.
...
You will likely only need to see some recent proved and unproved steps.
Another problem with long proofs is that they will consume my token quota.
So, you need to keep the number of visible steps at a minimum.
...
For example, instead of using the predefined set.mm variable `x`,
you can use a meaningful name like `set_of_all_sets`.
Whenever you think I or you would benefit from using meaningful variable names,
...
(taking into consideration all the strengths and weaknesses of the bottom-up prover explained in
the [Use bottom-up prover](#use-bottom-up-prover) section).
5. See what errors and unproved steps you have in the editor.
6. Fix errors. Invoke the `prove` function wherever needed.
Some steps will stay unproved because of missing disjoints.
You can invoke the `prove` function in that case; it will add the missing disjoints.
